...
rm -rf sim/simv* sim/csrc sim/DVEfiles sim/*.log
echo "=== Compiling I2C Controller ==="
echo "Start: $(date)"
vcs -sverilog     -f rtl/filelist.f     -f tb/uvm/filelist.f     -ntb_opts uvm-1.1     -cm line+cond+fsm+branch+tgl     -full64     -timescale=1ns/1ps     -top tb_top     -o sim/simv     2>&1 | tee sim/compile.log
exit ${PIPESTATUS[0]}
